...
with con:
    ...
    #data = con.execute("SELECT * FROM patient_data WHERE ((Diabetes = 'False') AND (TimeOfReporting = (TimeOfInfection + 2)) AND(RespiratoryIllness = 'False') AND (RespiratoryIllness = 'False') AND (AbnormalBloodPressure ='False')) AND (Outcome = 'Dead')")
    #print("number of ppl who died inspite of NOT having  diabetes,respillness,abnormalBP = ", len(data.fetchall()))
...
#data = combined_data_frame.groupby('TimeOfReporting')['xLocation'].count()
#print (data)
